...
def test_has_any_waitlist_returns_true_when_workshop_has_queue():
    repo = WaitlistRepository()
    repo.add_to_waitlist(10, 1)
    assert repo.has_any_waitlist(1) is True


def test_next_participant_returns_first_participant_in_queue():
    repo = WaitlistRepository()
    repo.add_to_waitlist(10, 1)
    repo.add_to_waitlist(40, 1)
    assert repo.next_participant(1) == 10
